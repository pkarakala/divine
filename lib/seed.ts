...
export async function seedDatabase() {
  ...
  for (const mockUser of MOCK_USERS) {
    ...
    const { error: userError } = await supabase.from('users').upsert({
      id: mockUser.id,
      email: `${mockUser.display_name.toLowerCase()}@divine-test.com`,
      is_verified: true,
    ...
    if (userError) console.log(`User error (${mockUser.display_name}):`, userError.message);
    ...
    if (profileError) console.log(`Profile error (${mockUser.display_name}):`, profileError.message);
...
export async function clearSeedData() {
  ...
  await supabase.from('prompts').delete().in('user_id', ids);
  await supabase.from('photos').delete().in('user_id', ids);
  await supabase.from('interactions').delete().in('sender_id', ids);
  await supabase.from('interactions').delete().in('receiver_id', ids);
